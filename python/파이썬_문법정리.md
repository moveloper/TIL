...
* java와 python 할당 순서의 차이 
  ...
  // 결과: 1
           2
  ```  
   `a = (b = c)`의 대입과정에서 `(b = c)`의 대입을 먼저 수행하지 않기 때문에(https://docs.python.org/ko/3/reference/expressions.html#evaluation-order  : 파이썬은 왼쪽에서 오른쪽으로 표현식의 값을 구합니다.), java처럼 오른쪽->왼쪽 할당연산자의 과정을 생각한다면 예상과 다른 결과를 얻을 수 있다.
  ```py
  >>>a=1
  >>>b=2
  >>>c=3
